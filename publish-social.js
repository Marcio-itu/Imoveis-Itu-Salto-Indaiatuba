...
//   2. IMOVEL_SLUG=algum-slug       (uma env var, um slug só)
//   3. IMOVEL_SLUGS="a b c"         (vários slugs separados por espaço, ex: saída de job do workflow)
...
const IG_USER_ID = process.env.INSTAGRAM_USER_ID || "17841455795855812";
...
function detectarSlugs() {
  ...
  if (process.env.IMOVEL_SLUG) return [process.env.IMOVEL_SLUG];
  if (process.env.IMOVEL_SLUGS) {
    return process.env.IMOVEL_SLUGS.split(/\s+/).filter(Boolean);
  }
  try {
