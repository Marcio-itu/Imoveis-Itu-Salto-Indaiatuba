...
function listarFotos(dados) {
  ...
  // O corretor agora escolhe manualmente, no admin, exatamente quais fotos vão pras redes
  // (marcando "usar nas redes sociais" em cada uma) — em vez do sistema adivinhar por
  // palavra-chave no nome do arquivo, que é frágil e já causou foto errada indo pro ar.
  const selecionadas = fotos.filter((f) => f.social);
  if (selecionadas.length) return selecionadas.slice(0, 10);

  // Fallback pra imóveis antigos, publicados antes dessa marcação existir: mantém o filtro
  // anti-rejeição por nome de arquivo (planta baixa / card comercial), que é o comportamento
  // que já existia. Assim que o imóvel for reeditado e ganhar fotos marcadas, para de usar isso.
  console.log("  aviso: nenhuma foto marcada \"redes sociais\" — usando filtro antigo por nome de arquivo como fallback");
  ...
  const listaFinal = fotosFiltradas.length >= 1 ? fotosFiltradas : fotos;
...
async function publicarNoCatalogo(slug, dados, fotos, config) {
  if (!CATALOG_ID) {
    log("  Catalogo: CATALOG_ID nao configurado - pulando");
    return null;
  }
  try {
    ...
    const graphUrl = GRAPH + "/" + CATALOG_ID + "/home_listings";
    const params = new URLSearchParams();
    params.append('access_token', ACCESS_TOKEN);
    params.append('home_listing_id', slug);
    params.append('name', (dados.titulo || slug).substring(0, 200));
    params.append('description', (dados.resumo || dados.descricaoCurta || dados.titulo || '').substring(0, 4999));
    ...
    params.append('price', String(precoNumerico));
    params.append('currency', 'BRL');
    params.append('url', dominio + "/imoveis/" + slug + "/");
    params.append('image[0][url]', urlFotoPrincipal);
    if (dados.quartos) params.append('num_beds', String(dados.quartos));
    if (dados.banheiros) params.append('num_baths', String(dados.banheiros));

    fotos.slice(1, 10).forEach((f, idx) => {
      const u = urlPublicaFoto(config, slug, f.arquivo);
      params.append("additional_images[" + idx + "][url]", u);
    });
    ...
    if (data.error) {
      if (/already exists|duplicate/i.test(data.error.message)) {
        log("  Info Catalogo: " + slug + " ja existe");
        return data;
      }
      throw new Error(data.error.message);
    }

    log("  OK Catalogo: " + slug + " enviado id=" + (data.id || slug));
    return data;
  } catch (err) {
    console.error("  Aviso Catalogo falhou (nao critico): " + err.message);
    return null;
  }
}
